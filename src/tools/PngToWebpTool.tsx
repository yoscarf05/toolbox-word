...
interface PngToWebpToolProps {
  lang: Language;
}

export const PngToWebpTool: React.FC<PngToWebpToolProps> = ({ lang }) => {
  ...
  const handleFile = (f: File) => {
    if (!f.type.includes('png') && !f.name.toLowerCase().endsWith('.png')) {
      setError(lang === 'es' ? 'Por favor sube una imagen en formato PNG.' : 'Please upload a PNG image.');
      return;
    }
    setError(null);
    setFile(f);
  ...
  const convertToWebp = (srcUrl: string, fileName: string, q: number) => {
    ...
    img.onerror = () => {
      setIsProcessing(false);
      setError(lang === 'es' ? 'Error al procesar la imagen PNG.' : 'Failed to process PNG image.');
    };
    img.src = srcUrl;
  };
    ...
    <div className="p-6 sm:p-10">
      <div className="flex items-center justify-between pb-6 mb-6 border-b border-slate-100 dark:border-slate-800">
        <div className="flex items-center gap-2 text-xs font-semibold text-emerald-600 dark:text-emerald-400">
          ...
          <span>{lang === 'es' ? 'Mantiene transparencia alfa al 100%' : '100% Alpha transparency retained'}</span>
        </div>
        <span className="text-xs text-slate-400">PNG → WebP</span>
      </div>
      ...
      {!file ? (
        ...
        >
          ...
          <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-1">
            {lang === 'es' ? 'Selecciona o arrastra tu archivo PNG' : 'Select or drag your PNG image'}
          </h3>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mb-6">
            {lang === 'es' ? 'Convierte a WebP conservando el fondo transparente intacto' : 'Preserve transparent backgrounds with much lower file size'}
          </p>
        ...
        <div className="space-y-6">
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 max-w-lg mx-auto">
            <div className="flex items-center justify-between text-xs font-bold mb-2">
              <span className="text-slate-700 dark:text-slate-300">
                {lang === 'es' ? 'Calidad WebP con transparencia:' : 'WebP Quality:'}
              </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="p-4 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-center space-y-2">
              <span className="text-xs font-bold text-slate-400">PNG Original ({formatSize(originalSize)})</span>
              <div 
                className="aspect-video rounded-xl overflow-hidden flex items-center justify-center p-2"
                style={{
                  backgroundImage: 'radial-gradient(#cbd5e1 1px, transparent 1px)',
                  backgroundSize: '12px 12px'
                }}
              >
                {previewUrl && <img src={previewUrl} alt="PNG" className="max-h-full object-contain" />}
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl border border-blue-200 dark:border-blue-900 bg-blue-50/20 dark:bg-blue-950/20 text-center space-y-2">
              <span className="text-xs font-bold text-blue-600 dark:text-blue-400">
                WebP Transparente ({formatSize(webpSize)}) {savings > 0 && `(-${savings}%)`}
              </span>
              <div 
                className="aspect-video rounded-xl overflow-hidden flex items-center justify-center p-2"
                style={{
              ...
              >
                {isProcessing ? (
                  ...
                  <img src={resultUrl} alt="WebP Transparente" className="max-h-full object-contain" />
                ) : null}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
            ...
            >
              ...
              <span>{lang === 'es' ? 'Descargar WebP Transparente' : 'Download Transparent WebP'}</span>
            </button>
            <button
              onClick={() => {
                setFile(null);
