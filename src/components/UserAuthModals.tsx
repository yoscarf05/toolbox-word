...
export const UserAuthModals: React.FC<UserAuthModalsProps> = ({ lang, onNavigate }) => {
  const {
    ...
    needsBootstrap: contextNeedsBootstrap
  } = useAuth();
  ...
  const resetAuthState = () => {
    ...
    setAuthMode(needsBootstrap || contextNeedsBootstrap ? 'bootstrap' : 'login');
  ...
  // Sync with context bootstrap state immediately
  useEffect(() => {
    if (contextNeedsBootstrap) {
      setNeedsBootstrap(true);
      setAuthMode('bootstrap');
    }
  }, [contextNeedsBootstrap]);
    ...
    try {
      if (authMode === 'register') {
        ...
        if (result.needsBootstrap) {
          ...
          setAuthMode('bootstrap');
          setErrorMsg(isEs 
            ? '¡Aún no hay ningún Super Administrador! Por favor completa los campos para crear la cuenta maestra inicial.' 
            : 'No Super Admin exists yet! Please complete the form to create the initial master account.');
          return;
        }
        if (!result.success) {
                ...
                <form onSubmit={handleBootstrapSubmit} className="space-y-4">
                  ...
                  <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
                    <button
                      ...
                      className="flex-1 py-2 text-xs font-bold rounded-lg bg-amber-600 text-white shadow-xs flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      <Sparkles className="w-3.5 h-3.5" />
                      <span>{isEs ? 'Super Admin Inicial' : 'Initial Super Admin'}</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="flex-1 py-2 text-xs font-bold rounded-lg text-slate-600 dark:text-slate-400 hover:text-slate-900 transition-colors cursor-pointer"
                    >
                      {isEs ? 'Iniciar Sesión' : 'Sign In'}
                    </button>
                  </div>
                ...
                <>
                    ...
                    <div className="p-3.5 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-amber-900 dark:text-amber-200">
                          {isEs ? '¡Primer Uso! Sistema sin Super Administrador' : 'First Launch! No Super Admin configured'}
                        </span>
                      </div>
                      <p className="text-[11px] text-amber-800/90 dark:text-amber-300/90 leading-relaxed">
                        {isEs
                          ? 'Aún no existe una cuenta maestra configurada en la base de datos. Para acceder al sistema debes registrar primero el Super Administrador inicial.'
                          : 'No master account exists yet in the database. You must register the initial Super Admin first to manage the system.'}
                      </p>
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="w-full py-2 px-3 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs shadow-xs cursor-pointer transition-colors flex items-center justify-center gap-1.5"
                      >
                        <Sparkles className="w-3.5 h-3.5" />
                        <span>{isEs ? 'Configurar Super Administrador Ahora' : 'Configure Super Admin Now'}</span>
                      </button>
                    </div>
                  )}
                  ...
                  <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-xl gap-1">
                    {needsBootstrap && (
                      ...
                      >
                        <Sparkles className="w-3.5 h-3.5" />
                        <span>{isEs ? 'Super Admin' : 'Super Admin'}</span>
                      </button>
                    )}
                    <button
                      onClick={() => {
